...
M.setup = function()
  ...
  local function handle_file_change(event_type, payload)
    ...
    local old_source_file =nil 
    local old_header_file =nil 


    if event_type == "new" or event_type == "delete" then
      source_file = payload.source_path
      header_file = payload.header_path
    elseif event_type == "rename" then
      source_file = payload.new_class_name.cpp
      header_file = payload.new_class_name.h
      old_source_file = payload.old_class_name.cpp
      old_header_file = payload.old_class_name.h
    elseif event_type == "move" then

      source_file = payload.operations[1].new
      header_file = payload.operations[2].new
      old_source_file = payload.operations[1].old
      old_header_file = payload.operations[2].old
    ...
    require("UEP.cmd.core.refresh_files").update_single_module_cache(module_name, function(ok)
      if ok then
        ...
        unl_events.publish(unl_event_types.ON_AFTER_UEP_LIGHTWEIGHT_REFRESH, {
          ...
          old_header_file = old_header_file,
