-- lua/UEP/provider/tree.lua (完成品のツリーモデルを返すプロバイダー)
...
local function build_fs_tree_from_flat_list(file_list, root_path)
  ...
  for _, file_path in ipairs(file_list) do
    ...
    -- root_pathからの相対パスを取得。区切り文字を考慮して+2
...
function M.build_tree_model(opts)
  ...
  if not game_data then
    return nil -- neo-tree側で "Please run :UEP refresh" メッセージを出す
  end

  local engine_data = engine_root and uep_project_cache.load(engine_root)
  
  -- ▼▼▼ ここからが、以前の提案で漏れていたフィルタリングロジックです ▼▼▼
  
  -- 1. 全モジュール情報を手動でマージ
  local all_modules_meta = {}
  -- まずEngine側のモジュールをすべて追加
  ...
  -- 次にGame側のモジュールを追加（もしキーが重複していたら、Game側の情報で上書きされる）
  ...
  if not next(all_modules_meta) then
      return nil -- モジュールが一つもなければnilを返す
  end

  -- 2. optsに基づいて表示対象モジュールを決定
...
-- requestメソッドが、optsの内容に応じて処理を振り分ける
