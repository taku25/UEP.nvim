-- lua/UEP/cmd/add_include.lua (RPC Optimized)
...
local function insert_include_for_class(target_class_name)
  ...
  unl_api.db.find_class_by_name(target_class_name, function(class_info, err)
      if err or not class_info then
  ...
  if opts.has_bang then
    ...
    derived_core.get_all_classes({}, function(all_classes)
      if not all_classes or #all_classes == 0 then
        return log.error("No classes found. Please run :UNL refresh.")
      end
