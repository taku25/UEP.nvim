-- lua/UEP/cmd/structs.lua (新スコープ・Depsフラグ対応版)
...
function M.execute(opts)
  ...
  if opts.scope then
      ...
      if valid_scopes[scope_lower] then
          ...
          log.warn("Invalid scope argument '%s' for structs command. Defaulting to 'runtime'.", opts.scope)
      end
  end
  ...
  if opts.deps_flag then
      ...
      if valid_deps[deps_lower] then
          ...
          log.warn("Invalid deps flag '%s' for structs command. Defaulting to '--deep-deps'.", opts.deps_flag)
      end
  end
  -- ▲▲▲ 引数パース修正ここまで ▲▲▲

  log.debug("Executing :UEP structs with scope=%s, deps_flag=%s, bang=%s",
